...
{
    private $model = Barrio::class;
    private $table = 'barrio';
    private $route = "tablasMaestras.barrio";
    private $campos = ['descripcion', 'rela_localidad'];
    ...
    public function indexApi()
    {
        ...
        if(!empty($filtro)){
            ...
            $query->where(function($q) use ($filtro, $campos) {
                ...
                $q->orWhereHas('localidad', function($sub) use ($filtro) {
            ...
            'descripcion' => [
                ...
                ->where(function ($query) use ($request) {
                    return $query->where('descripcion', $request->input('descripcion'))
                                 ->where('rela_localidad', $request->input('rela_localidad'));
                })
                ->ignore($id),
            ],
            'rela_localidad' => 'required|integer|exists:localidad,id',
        ];

        $messages = [
            'descripcion.required'     => 'La descripción es obligatoria.',
            'descripcion.unique'       => 'Ya existe un barrio con esa descripción.',

            'rela_localidad.required'  => 'Debés seleccionar una localidad.',
            'rela_localidad.integer'   => 'La localidad no es válida.',
            'rela_localidad.exists'    => 'La localidad no existe.',
        ];
